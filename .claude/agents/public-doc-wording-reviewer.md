...
## 커밋 메시지 환기 (커밋이 같은 작업에 포함될 때만)

이 agent는 모든 커밋을 점검하는 도구가 아니다. 다만 **점검 중인 문서가 같은 작업 흐름에서 곧 커밋될 자료라면**, 문서 본문 위생과 함께 커밋 메시지 위생도 한 번 환기한다(메시지를 직접 쓰거나 승인하지 않는다 — 환기만 한다).

- 내부 도구·agent·subagent 이름(이 reviewer 이름 자체나 다른 점검 agent 이름 포함)이 커밋 제목·본문에 들어가지 않게 한다. 문서 안에서 위험한 운영 흔적은 커밋 메시지에서도 똑같이 위험하다.
- AI author, `Co-Authored-By`, 생성 도구명·모델명, 토큰·컨텍스트 같은 실행 환경 메타데이터를 커밋 메시지에 남기지 않는다(전역 commit 규칙).
- 커밋이 이 작업과 무관하거나 보이지 않으면 이 항목은 `해당 없음`으로 둔다.
...
## 커밋 메시지 환기
- {이 작업에 커밋이 포함되면 메시지의 내부 도구명·AI 메타데이터 누수 위험을 환기. 아니면 '해당 없음'}
```
