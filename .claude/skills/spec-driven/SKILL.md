---
name: spec-driven
description: TripProof repo 전용 light spec-driven 작업 루프. TripProof에서 큰 작업, AI 위임, AC(acceptance criteria) 선택, product-first/eval 관찰, 사람 판단 회수, spec/decision/work-log 기록 위치를 판단해야 할 때 사용한다. Scope를 줄이기 전에 사용자가 넣은 자료가 근거/후보, 답변/상태, 화면으로 이어지는 제품 흐름이 끊기지 않는지 본다. "TripProof spec-driven", "feature spec", "AC를 고르자", "spec으로 작업하자", "이 작업이 큰 작업인가", "문서가 실행을 잡아먹지 않게" 같은 요청에 트리거한다.
---
...
작게 자르기 전에 줄이면 안 되는 제품 흐름을 먼저 본다. 줄일 수 있는 것은 범위, 필드 수, provider 품질, ingestion 충실도, 화면 다듬기다. 줄이면 안 되는 것은 `사용자 자료 -> 근거/후보 -> 답변/상태 -> 화면`의 흐름이다. 작은 UI 조정이나 기존 AC 안의 bugfix는 이 흐름을 머릿속으로만 확인하고 바로 진행한다.

제품 흐름을 어디서 잘라도 되는지 애매하면 임의로 `다음 slice`나 stub으로 넘기지 않는다. 특히 앞 단계 산출물을 raw로 보여줄지, 이번 단계의 사용자-facing 결과로 변환해야 할지 헷갈리면 사용자에게 짧게 확인한다.
...
- 제품 계약, evidence state, human review, card 승격, eval 관찰 대상이 바뀐다.
...
큰 작업이 큰 feature면, AC로 바로 좁히기 전에 구현면을 먼저 펼친다. 업로드, 텍스트 추출, 청크, 후보 생성, 근거, 답변 UI, 카드, 대시보드, guard 같은 구현면을 꺼내 보고, 그 면을 작게 관통하는 slice 후보를 본다. 여기서 slice는 작은 TODO나 가장 빨리 보이는 화면 조각이 아니라 제품 흐름이다. 불확실하다고(PDF/OCR/LLM) 미리 Non-goals로 자르지 않는다. 이미 경로가 좁은 작은 spec이나 bugfix라면 생략할 수 있고, 생략할 때는 왜 생략하는지 한 줄로 둔다.

LLM 불확실성은 데모로 대체하지 않는다. 줄일 수 있는 것은 provider 품질, ingestion 충실도, 처리 범위이지, `자료 -> 후보/evidence -> 상태 -> 화면`으로 이어지는 제품 흐름이 아니다. 여기서 제품 흐름은 자료가 바뀌면 답도 바뀌고, evidence quote가 없으면 `근거 있음`이 될 수 없는 연결이다. deterministic baseline이나 stub은 같은 계약을 통과시키는 test double일 수 있지만, 제품 확인의 주인공이 되면 안 된다.

앞 단계 산출물을 소비하는 slice에서는 raw 산출물을 그대로 노출할지, 이번 단계에서 사용자가 읽는 결과로 변환해야 할지 먼저 구분한다. 예: `FactCandidate[] -> ChatAnswer -> 채팅 UI`. 이 예시는 04 채팅 계열의 calibration이며, 다른 feature에서는 해당 feature의 출력 계약으로 치환한다. raw candidate, fixture value, retrieval/debug output을 화면에 붙이는 것은 제품 slice가 아니라 debug 표시다.

## Light Brief

큰 작업은 아래 5-6줄 정도로 시작하고, 제품 흐름이 흔들릴 수 있으면 `제품 흐름` 한 줄만 더한다. 기본 출력 위치는 대화나 현재 작업 메모다. 별도 파일은 기록 위치 조건을 만족할 때만 만들고, PR/commit 근처 기록은 실제로 PR이나 commit을 다룰 때만 쓴다. 큰 feature라면 구현면과 slice 후보를 본 뒤 `먼저 고를 slice`를 같이 적는다.
...
큰 작업이면 위 필드를 한 줄씩만 채운다. 작은 작업이나 bugfix에서는 제품 흐름을 머릿속으로 확인하고 바로 실행해도 된다. `이번 AC`는 기본 1-3개로 제한한다. 4개 이상이면 feature가 너무 커졌는지 먼저 의심하되, 숫자보다 한 사용자 장면이나 한 실패 유형으로 관찰 가능한지를 먼저 본다. AC를 줄인다는 이유로 그 behavior를 성립시키는 흐름을 자르지 않는다.

좋은 좁힘과 나쁜 좁힘의 기준이 상황마다 달라질 수 있다. 제품 흐름에서 어느 연결을 이번에 유지해야 하는지 확신이 없으면, 하네스가 대신 결정하지 말고 사용자에게 확인한다.

예시:

```text
왜 지금: 첫 제품 확인을 숙소 체크인 준비 장면으로 좁혀, 문서/하네스가 아니라 화면 동작으로 확인한다.
...
제품 흐름: material text -> evidence-backed candidate -> supported/missing answer state -> 채팅 답변 / raw output 렌더로 대체하지 않는다.
...
7. Slice 후보: 펼친 구현면을 관통하는 제품 흐름 후보는 무엇인가.
8. 먼저 고를 slice: 이번 구현에서 화면까지 이을 경로 하나는 무엇인가.
9. 이번 AC / 확인 방법: 어떤 제품 동작을 관찰할 것인가.
...
스펙을 구현하기 전에는 먼저 해당 스펙이 단독 기준인지, 상위 feature spec이나 앞뒤 하위 스펙처럼 함께 읽어야 할 참조 맥락이 있는지 확인한다. 참조 맥락이 있으면 부모 스펙의 사용자 장면, 먼저 고를 slice, 이번 AC와 인접 스펙의 입력/출력 계약을 짧게 본다. 목적은 관련 문서를 작업 목록으로 만드는 것이 아니라, 현재 구현할 스펙이 제품 흐름 안에서 어떤 입력을 받고 어떤 출력을 넘기는지 확인하는 것이다. gate가 아니라 짧은 context pass다.

짧게 확인할 질문:

- 현재 스펙은 단독 구현 기준인가, 상위/인접 스펙을 참조해야 하는가?
- 입력은 앞 단계에서 온 제품 산출물인가, fixture/seed인가?
...
- raw output, fixture, debug 응답, API 응답 그대로 보여주기가 사용자가 읽는 결과로 변환하는 일을 대체하고 있지 않은가?
- 제품 흐름을 어디서 자를지 애매한데 AI가 임의로 다음 slice나 stub으로 넘기고 있지 않은가?
...
- API 응답을 그대로 보여주는 것이 slice 축소처럼 제안된다.
- 사용자가 읽는 결과 이름 없이 upstream 상태명이나 raw field만 화면 props로 남는다.
- `LLM은 나중에`가 `retrieval/grounding 계약도 나중에`로 미끄러진다.

## TripProof의 기본 경계

- product가 먼저다. eval은 제품 동작을 관찰한다.
...
- raw output, fixture value, debug response를 화면에 붙이고 제품 slice가 닫혔다고 말한다.
- `더 얇게`가 어떤 계약은 유지하고 어떤 기술 충실도만 줄이는지 없이 쓰인다.
- 좋은 좁힘과 나쁜 좁힘이 애매한데 사용자 확인 없이 AI가 흐름을 잘라낸다.
- LLM hallucination, PDF/OCR 복잡도, 민감정보 위험을 이유로 `seed -> hard-coded answer -> 화면`만 만들고 제품 확인이라고 부른다.
...
이 섹션은 현재 첫 제품 확인을 고를 때의 calibration이다. 현재 작업 queue가 아니며, 다른 feature에는 숙소 체크인 예시가 아니라 같은 판단축만 옮긴다.

TripProof의 첫 제품 확인은 기능 전체보다 실패 유형 단위로 좁힌다.
...
slice 후보는 그 구현면을 작게 관통하는 제품 흐름이다. 예:
...
- 화면에 도달한 값이 제품 흐름을 통과했는지가 완료 기준이다.
