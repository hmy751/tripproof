...
Spec/AC는 이번 slice의 product contract다. `docs/engineering/`은 그 contract를 구현할 때 쓰는 설계 감각 기준이며, spec을 대체하거나 상위 gate가 되지 않는다. spec 적용 중 product/eval boundary, testing level, shared contract, AI coding workflow 같은 위험 신호가 보이면 관련 engineering 문서만 targeted read한다.
