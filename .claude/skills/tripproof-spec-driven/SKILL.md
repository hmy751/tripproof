...
큰 작업이 큰 feature면, AC로 바로 좁히기 전에 구현면을 한 번 펼쳐 봐도 된다. 업로드, 텍스트 추출, 청크, 후보 생성, 근거, 답변 UI, 카드, 대시보드, guard 같은 구현면을 꺼내 보고, 그 면을 얇게 관통하는 slice 후보를 본다. 불확실하다고(PDF/OCR/LLM) 미리 Non-goals로 자르지 않는다. 펼치는 건 선택이지 거쳐야 하는 절차가 아니다.

## Light Brief

큰 작업은 아래 5-6줄 정도로 시작한다. 별도 파일이 아니라 대화, 작업 메모, PR/commit 근처에 있어도 된다. 큰 feature라면 구현면과 slice 후보를 본 뒤 `먼저 고를 slice`를 같이 적는다.
...
큰 feature를 펼친 뒤에는 한 slice씩 짓는다. 순서는 `구현면 펼치기 -> slice 후보 -> 먼저 고를 slice`다.
