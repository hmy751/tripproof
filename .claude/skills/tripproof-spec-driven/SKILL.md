...
큰 작업이 큰 feature면, AC로 바로 좁히기 전에 구현면을 한 번 펼쳐 봐도 된다. 업로드, 텍스트 추출, 청크, 후보 생성, 근거, 답변 UI, 카드 같은 구현면을 꺼내 보고, 불확실하다고(PDF/OCR/LLM) 미리 Non-goals로 자르지 않는다. 펼치는 건 선택이지 거쳐야 하는 절차가 아니다.

## Light Brief

큰 작업은 아래 5줄 정도로 시작한다. 별도 파일이 아니라 대화, 작업 메모, PR/commit 근처에 있어도 된다.
...
`이번 AC`는 기본 1-3개로 제한한다. 4개 이상이면 feature가 너무 커졌는지 먼저 의심한다.
...
남은 판단: P0는 실제 PDF/OCR ingestion 없이, synthetic text와 stub adapter(deterministic baseline)로 한 필드를 화면까지 닿게 시작한다.
```
...
```text
사용자 장면 선택
-> 현재 코드와 비교해 이미 되는 것, stub으로 둘 것, 이번에 이어야 할 경로를 짧게 확인
-> 그 확인을 바탕으로 light brief와 이번 AC 1-3개 먼저 제시
-> product 구현
-> 수동 확인 또는 테스트 관찰
-> 필요한 판단만 기록
```
...
펼친 후보가 크고 독립적이면 별도 feature spec(`01` → `01-01`)으로 나눌 수 있다. 대부분의 후보는 slice라 한 feature 안에서 순서대로 짓는다.
...
6. 이번 AC: 지금 작업에서 고를 1-3개.
7. 확인 방법: 테스트나 수동 product 확인으로 관찰할 것.

세부 기준, entry point, Tests / scenario(사례), eval 관찰 기준은 그 뒤에 둔다. scenario는 작업 순서표가 아니라 AC를 확인할 때 꺼내는 대표 상황으로 읽는다.

feature spec이 전체 기준을 담고 있어도, 현재 구현 작업은 별도 brief에서 `이번 AC` 1-3개를 다시 고른다. 전체 기준의 확인 방법은 feature가 언젠가 닫혔다고 말하기 위한 기준이지, 모든 작업의 기본 체크리스트가 아니다.
...
큰 feature를 펼친 뒤에는 한 slice씩 짓는다.

구현면을 펼칠 때는 상상한 기능 목록보다 현재 코드베이스와 먼저 비교한다. 이미 되는 것, 이번에 stub으로 둘 것, 사용자 장면을 통과시키려면 이어야 할 부분을 짧게 잡는다. 이 확인은 표나 work-map의 기본 형식이 아니라, 이번 light brief와 AC를 정확하게 잡기 위한 재료다.

- 각 slice는 한 필드를 실데이터로 끝까지 통과시킨다 (자료 → AI → 상태 → 화면). 여기서 "끝까지"는 화면에 닿는 깊이지, 모든 부품을 완성한다는 뜻이 아니다. 나머지 부품과 다른 필드는 stub으로 둔다. 부품 목업을 따로 만들지 않는다. 이것이 목업 병목을 막는 기준이다.
- 완료 기준은 컴포넌트가 아니라 화면 동작이다.
- 닫은 slice는 commit 본문에 AC 태그로 남긴다 (예: `... (AC #1)`).

실제 ingestion(PDF/OCR/Gmail), `conflict`, `sensitive`, metric threshold는 보통 product 흐름이 돈 뒤 별도 slice로 열 수 있다. 다만 이것을 금지 목록으로 쓰지 않는다. 이번 사용자 장면이 화면까지 닿으려면 어떤 대체 입력, adapter stub, 상태 연결이 필요한지 현재 코드와 비교해 정한다.
