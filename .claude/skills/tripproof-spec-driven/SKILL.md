...
3. Non-goals: 이 feature를 작게 유지하기 위해 의도적으로 빼는 것은 무엇인가.
4. 상태 언어: 사용자에게 보이는 말과 내부 상태 축의 관계.
5. 이번 AC: 지금 작업에서 고를 1-3개.
...
- 각 slice는 한 필드를 실데이터로 끝까지 통과시킨다 (자료 → AI → 상태 → 화면). 나머지 부품은 stub으로 둔다. 부품 목업을 따로 만들지 않는다. 그게 목업 병목이다.
