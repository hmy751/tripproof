...
- `docs/` — 필요한 설계 메모와 결정 기록. 제품 어휘·상태·흐름의 단일 기준은 `docs/product-model.md`다.
- `docs/product-model.md` — 제품 통합 모델 기준 문서. chat-first 흐름, 객체 모델, 상태 2축(근거 축·결정 축)을 한 번만 서술하고 PRD/spec/README가 참조한다.
- `docs/roadmap/` — 로드맵 버전 보관(현재 기준 `v1.md` = 1차 로드맵·MVP 계획). 변하지 않는 목표, MVP가 통과시킬 한 흐름, 확장 후보 목록을 느슨한 메뉴로 두어 다른 세션이 단독으로 작업을 고르게 한다. 고정 계획·일정표가 아니라 갈아엎을 수 있는 초안.
- `docs/archive/` — 동결 스냅샷·참조 보관소(활성 문서 아님, 갱신하지 않음). `docs/archive/preview/`에 통합의 출발점이 된 chat-first UX 원천(`prd.md`, `tripproof-preview-c.html`)을 보존한다. 현재 기준은 `docs/product-model.md`다.
