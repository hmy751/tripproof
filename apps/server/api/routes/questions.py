...
from server.use_cases.questions import (
    AskQuestionCommand,
    AskQuestionUseCase,
    EmptyQuestionError,
)
...
def ask_question(
    ...
    answer_composer: Annotated[
        LibraryChatAnswerComposer, Depends(get_library_chat_answer_composer)
    ],
    observation_sink: Annotated[
        QuestionObservationSink, Depends(get_question_observation_sink)
    ],
    ...
    try:
        result = use_case.run(
            AskQuestionCommand(
                question=payload.question, material_ids=payload.material_ids
            )
        )
