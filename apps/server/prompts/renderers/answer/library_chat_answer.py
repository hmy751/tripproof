...
class LibraryChatAnswerPrompt:
    ...
    def user_message(self, *, question: str, source_blocks: str) -> str:
        template = _required_section(markdown=self.document.body_markdown, heading="User")
        return _render_template(template, question=question, source_blocks=source_blocks)

    def snapshot(self) -> dict[str, str]:
        return self.document.snapshot()


def load_library_chat_answer_prompt(version: str = LIBRARY_CHAT_PROMPT_VERSION) -> LibraryChatAnswerPrompt:
...
def _required_section(*, markdown: str, heading: str) -> str:
    ...
    if match is None:
        raise ValueError(f"Library Chat answer prompt에 필요한 section이 없습니다: {heading}")
    body = match.group("body").strip()
    if not body:
        raise ValueError(f"Library Chat answer prompt section 본문은 비어 있을 수 없습니다: {heading}")
    return body
...
def _render_template(template: str, **values: str) -> str:
    ...
    if missing:
        raise ValueError(f"Library Chat answer prompt 렌더링 값이 없습니다: {', '.join(missing)}")
    return rendered
