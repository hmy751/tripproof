...
| `src/ai/` | Python AI 후보 생성 코드와 prompt/provider 자리. 최종 product 계약이 아니라 server가 정규화할 후보 JSON을 만든다 |
| `src/server/` | product entry point와 서버 내부 구현 |
| `src/server/trip-facts/` | Python/LLM 후보를 `shared` 계약으로 정규화하는 TS server 경계 |
