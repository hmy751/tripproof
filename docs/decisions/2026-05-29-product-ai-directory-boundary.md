...
> 상태: 2026-06-08 `docs/decisions/2026-06-08-python-backend-uv-ingest-boundary.md` 결정으로 대체됨. 현재 구조는 root `client/`와 root `server/`를 기준으로 한다.
...
비교 기준으로 `pilab-multimodal-rag`를 확인했다. 그 프로젝트는 `app/`과 `evals/`, `experiments/`를 분리하고, 모델 호출과 검색/선별/방어 규칙이 `app/` 내부에 함께 존재한다. TripProof도 같은 문제의식을 따른다.

다만 TripProof는 초기 단계라 `pilab`처럼 세부 레이어를 많이 만들지 않고, product-first 경계만 유지한다.
