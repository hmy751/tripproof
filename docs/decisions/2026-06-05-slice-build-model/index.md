...
2. **큰 feature spec 안에 slice 목록을 둘 수 있다.** 단 이 목록은 진행 메모(상기용)지 통과 기준이 아니다. 비어 있어도 코딩을 막지 않고, slice가 AC를 새로 정의하지 않는다. 별도 work-map 파일은 만들지 않는다.
3. **빌드 방법:** AC로 바로 좁히기 전에 구현면을 한 번 펼쳐(불확실한 PDF/OCR/LLM도 미리 Non-goals로 안 자름) 후보를 본다 → 후보를 slice로 **한 필드씩 실데이터로 끝까지 통과**시킨다(자료 → AI → 상태 → 화면) → 나머지 부품은 stub, 부품 목업은 따로 만들지 않는다(목업 병목 차단) → 완료 기준은 컴포넌트가 아니라 화면 동작이다 → 닫은 slice는 AC 태그로 commit한다.
...
- slice 목록이 통과 기준처럼 굳거나 코딩을 막기 시작한다.
