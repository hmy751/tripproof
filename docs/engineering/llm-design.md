...
- **그 코드 계약의 판정 기준은 단어가 아니라 구조여야 한다.** 의미 분류("이 질문이 무엇을 요구하나, 이 문장이 조건인가")는 LLM·relation extractor의 몫이고, 코드는 source unit `kind`/metadata와 근거 set 구성 같은 구조 사실만 본다.
    - **나쁜 예** — 답변에 "확정"이 보이면 강등하거나, 질문에 "취소·환불" 키워드가 보이면 조건 근거를 요구한다. 답변 단어든 질문 단어든 *의미를 단어로 환원*한 것이라, 같은 의미를 키워드 없이 다르게 물으면(paraphrase) 그대로 샌다. 코드가 기댄 `kind` 신호마저 lexical 산출이면, 놓쳤을 때 default가 위험한 쪽(`supported`)으로 떨어진다.
    - **좋은 예** — "값-only source unit은 조건·caveat kind 근거가 함께 인용돼야 `supported`"처럼 질문 문구와 무관한 구조 규칙. 구조 신호가 없거나 애매하면 default는 `needs_review`(모르면 확정하지 않는다).
    - **질문** — 같은 의미를 키워드 없이 다르게 물으면 결과가 바뀌나? 바뀌면 단어에 기댄 것이다. 게이트가 source unit kind·evidence set 구성에 걸리나, 텍스트 단어에 걸리나?
