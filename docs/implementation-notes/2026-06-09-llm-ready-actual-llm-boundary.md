...
숙소 체크인 03 구현 중 `RAG -> LLM/extractor -> validator -> fact candidate` 경계를 정리했지만, 당시 코드에서는 LLM provider를 호출하지 않고 `CheckinFactProposer` protocol과 local proposer로 후보 생성을 닫았다.
...
당시 구현에서 확인된 경계는 다음과 같았다.

- `retrieve_context`는 source unit 후보를 고른다.
- local proposer는 후보 원문 안에서 코드 내부 규칙 기반 proposal을 만든다.
- `validate_fact_proposal`은 proposal의 snippet이 source unit 원문 일부인지 확인한 뒤에만 `supported`로 승격한다.
- `apps/server/llm/client.py`에는 structured output client protocol만 있고, 체크인 03 경로에서 실제 provider call은 없었다.

따라서 당시 구현은 `RAG + local proposer + grounding validator`로 분류해야 하며, 실제 LLM 판단 경로로 표현하면 안 된다.

## 후속 구현 반영

이 관찰은 이후 03 구현에서 반영됐다. 후속 check-in fact candidate 기본 경로는 Supabase vector retrieval로 만든 `ContextPack`을 Ollama JSON proposer에 넘기고, validator가 source unit 원문으로 grounding된 proposal만 `supported`로 받아들인다. 테스트와 비활성 모드에서는 deterministic proposer나 missing proposer를 주입하지만, 제품 기본 경로를 local proposer로 닫지 않는다.
...
이 경계는 "문자열 기반 보조 로직을 절대 쓰면 안 된다"가 아니다. 다만 코드 내부 규칙 proposal이 실제 LLM 판단을 대신했으면 그 구현은 LLM 통합 완료가 아니라 local proposer 구현이다. 현재 check-in fact 생성 기본 경로에는 코드 내부 규칙 proposal을 두지 않는다.
