...
새 implementation note는 기본적으로 폴더형으로 남긴다.

- `YYYY-MM-DD-짧은-주제/index.md`: 독립적으로 읽히는 관찰, 다시 볼 경계, 기록 위치 판단.
- `YYYY-MM-DD-짧은-주제/raw.md`: 대화·조사·검토에서 나온 배경 재료. 현재 실행 기준이나 작업 대기열이 아니다.

다음 세션은 먼저 `index.md`를 읽는다. `raw.md`는 왜 그런 관찰이 생겼는지 필요할 때만 본다. `raw.md`만 읽어야 알 수 있는 결론은 남기지 않는다.
...
- 폴더형 노트의 `raw.md`에도 대화 원문을 그대로 붙이지 않는다. 배경 재료는 역할과 판단 축이 드러나게 중립적으로 요약한다.
