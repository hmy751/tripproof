...
| --- | --- | --- |
| Trip | 확인하려는 하나의 여행 묶음 (오사카 4박 5일) | UI/preview 개념. 확장 후보. |
...
- `ChatAnswer`/`TripFact`에 해당하는 backend response schema를 02 slice에서 새로 잡는다.
- `value`는 `missing`/`conflict` 후보를 값 없이 표현하려면 `string | null` 형태가 필요할 수 있다(slice spec도 이미 지적).
- 대시보드 2차 분류축을 계약으로 올리려면 `category` 필드가 필요하다.
- 카드 출처(결정 축)를 backend/eval이 읽어야 하면 `ReviewDecision` 또는 카드 출처 필드를 API schema에 추가한다.
- conflict 표현을 위한 `conflictGroupId`, `conflictCandidates`, fact-level `reason`은 P0 출력 계약에서 필요해지면 작게 확장한다.
- AI confidence 수치는 사용자 언어로 쓰지 않는다(아래 5. 정직성 원칙).
...
`supported origin`은 별도의 새 출처 라벨을 붙이는 게 아니다. 근거 축의 `supported`가 사람의 수정 없이 카드까지 보존된 경우이며, preview HTML도 답변과 같은 `근거 있음` pill을 카드에 그대로 재사용한다. 그래서 '근거 있음'이라는 한 단어가 두 축에 동시에 나타나지만 직교성을 깨지 않는다 — 사람이 값·이름을 고치는 순간 `직접 확인`으로 바뀌기 때문이다. (분리가 필요한 건 `확인 필요` ⟂ `직접 확인`이다.)
...
| 요구·acceptance·AI behavior·도메인 타입 이름·eval 축 | `docs/prd.md`, 필요할 때 새로 여는 `docs/specs/*.md` | 내용 유지 + 기준 문서 어휘/상태 2축으로 정렬. AI Must Not(근거 없는 답 금지, 충돌 뭉개기 금지, 민감정보 자동저장 금지) 보존 |
| 프로세스·spec 운영·결정 | `docs/specs/README.md`, `docs/decisions/`, `docs/development-notes.md` | docs 유지. product/eval 기본 방향 노트(`development-notes.md`)는 그대로 두고, 흐름 서술이 기준 문서와 겹치면 기준 문서를 참조 |
| 도메인 타입 정의 (실제 코드 계약) | `server/models.py` + `src/client/types.ts` | backend API schema를 기준으로 하고 client type은 그 응답을 좁게 반영한다. 02 이후 답변·근거·카드 schema를 작게 확장한다 |

이 기준 문서는 product가 먼저고 eval은 product를 관찰만 한다는 원칙(`docs/specs/README.md`)을 보존한다. eval 축 이름은 그대로 유지한다: Faithfulness/Groundedness, Citation Precision, Abstention F1, Conflict Recall.
