...
상태: 계획 문서. 아직 product flow, eval run, before/after 결과가 있다는 뜻이 아니다.
...
문서에 남길 수 있는 것은 현재 repo에서 확인 가능한 목표, 제약, 결정, 열린 질문, 다음 검증이다. 아직 구현되지 않은 product flow, eval run, before/after 결과는 완료된 것처럼 쓰지 않는다.
...
| `fixtures/accommodation-checkin/` case 자료 | 첫 product slice를 통과시키기 위한 synthetic input | product contract를 확인할 최소 case가 정리됨 |
| `docs/work-log.md` entry | 중요한 판단의 재진입 기록 | 왜 열었고 다음이 무엇인지 2-4줄로 남음 |
...
P0 fixture에는 happy path, missing late arrival, multi-doc supplement, conflict check-in time, sensitive guard를 넣는다. 구현은 현재 구조에 맞춰 `src/server/ai/extractTripFacts.ts`, `src/server/ai/normalizeTripFacts.ts`, `src/shared/tripFacts.ts`를 확장하는 deterministic baseline으로 시작한다. LLM adapter는 P0 이후 별도 slice로 연다. review UI는 이미 있는 `src/client/` 뼈대를 먼저 현재 contract에 맞춰 조정한다.

## 최소 루프

각 큰 작업은 아래 순서를 따른다.

1. 작업 brief: 왜 지금 이 slice를 여는지, 무엇을 통과로 볼지 적는다.
2. Product contract: product가 반환할 최소 구조를 잡는다.
3. Fixture: synthetic text fixture로 happy path와 failure path를 만든다.
4. Product behavior: 사용자가 볼 결과 계약을 먼저 통과시킨다.
5. Observation: test 또는 eval은 그 결과를 호출해 관찰한다.
6. Record: 채택, 기각, 보류, 다음 확인을 가까운 곳에 남긴다.
