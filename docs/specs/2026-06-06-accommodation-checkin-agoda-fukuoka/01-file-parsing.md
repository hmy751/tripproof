...
제품의 시작은 사용자가 원본 자료 파일을 넣고, 제품이 그 파일에서 본문을 얻는 것이다. 이 단계는 Agoda 예약 확정 PDF 하나를 제품에 넣고, 파싱된 본문이 자료함과 다음 `SourceUnit`/`EvidenceRef` grounding 단계 입력으로 이어지는 데까지를 닫는다.
...
- 파싱된 본문은 `SourceUnit`/RAG search와 `ChatAnswer`/`EvidenceRef` grounding 단계가 읽을 수 있는 상태로 저장된다.
- 파싱 본문에는 예약 확정서 제시 안내, 체크인/체크아웃 날짜, 취소/노쇼 정책, 도시세 안내, 신분증/카드 제시 안내가 들어간다.
- 검증된 `EvidenceRef`로 확인되지 않은 체크인 관련 값을 성공한 것처럼 만들지 않는다.
...
4. `SourceUnit`/`EvidenceRef` grounding 입력에는 실제 PDF에서 파싱된 본문이 들어간다.
5. 체크인 시작 시각에 대한 검증된 `EvidenceRef`가 없으면 값을 임의로 넣지 않는다.
...
- 질문 API는 ready material의 파싱 본문이 다음 `SourceUnit`/`EvidenceRef` grounding 입력으로 넘어가는지만 01에서 확인한다. `SourceUnit`, `EmbeddingRecord`, `RetrievedSource`/`AnswerContext`, `ChatAnswer`, `EvidenceRef`, 인라인 근거 UI, 카드 초안은 02 이후에서 다룬다.
...
- 질문 API는 ready material의 파싱 본문을 question 입력으로 받는 데까지만 닫았다.
