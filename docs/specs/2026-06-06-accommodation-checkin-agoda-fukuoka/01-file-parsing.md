...
- 파싱된 본문은 source unit/retrieval과 evidence-backed fact 단계가 읽을 수 있는 상태로 저장된다.
- 파싱 본문에는 예약 확정서 제시 안내, 체크인/체크아웃 날짜, 취소/노쇼 정책, 도시세 안내, 신분증/카드 제시 안내가 들어간다.
- accepted source로 확인되지 않은 체크인 관련 값을 성공한 것처럼 만들지 않는다.
...
5. 체크인 시작 시각에 대한 accepted source가 없으면 값을 임의로 넣지 않는다.
...
- 질문 API는 ready material의 파싱 본문이 다음 source/evidence 입력으로 넘어가는지만 01에서 확인한다. source unit, retrieval 후보, fact, 답변 문장, 인라인 근거 UI, 카드 초안은 02 이후에서 다룬다.
