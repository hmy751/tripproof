...
- 파싱된 본문은 source unit/RAG index와 evidence-backed fact 단계가 읽을 수 있는 상태로 저장된다.
...
- 질문 API는 ready material의 파싱 본문이 다음 source/evidence 입력으로 넘어가는지만 01에서 확인한다. source unit, index record, retrieval 후보, fact, 답변 문장, 인라인 근거 UI, 카드 초안은 02 이후에서 다룬다.
