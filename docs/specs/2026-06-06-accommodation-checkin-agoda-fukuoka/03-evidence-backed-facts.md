# 숙소 체크인 03 - Evidence-backed fact와 상태
...
source unit과 accepted source가 있어도, 그것만으로 제품 답변이 되지는 않는다. 이번 단계는 02에서 찾은 accepted source를 `TripFact`, `EvidenceRef`, `EvidenceState`로 바꾸고, accepted source가 없는 값은 만들지 않는 상태 계약을 닫는다.
...
- evidence-backed fact 생성 입력은 02의 accepted source를 받는다.
...
- 체크인 시작 시각은 accepted source가 없으면 value 없이 `근거 부족`으로 남는다.
...
- `supported`는 accepted source와 `EvidenceRef` 없이는 나올 수 없다.
- `EvidenceRef.snippet`은 실제 accepted source text의 일부여야 한다.
...
- rejected candidate를 `EvidenceRef`로 쓰지 않는다.
...
- `apps/server/retrieval/`: 02 source unit과 retrieval 후보 경계가 들어갈 자리.
...
```text
accepted source / missing target
...
1. 예약 확정서 제시 안내는 accepted source가 있을 때만 `근거 있음` fact로 나온다.
2. 예약 확정서 제시 근거는 Agoda PDF에서 파싱한 본문 일부를 보여준다.
3. 체크인 시작 시각은 accepted source가 없으면 value 없이 `근거 부족` fact로 나온다.
4. 체크인 제시물 accepted source가 없으면 해당 fact가 `근거 있음`으로 나오지 않는다.
