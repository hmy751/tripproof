...
사용자가 체감하는 첫 제품 동작은 채팅 답변이다. 이번 단계는 자료함에 질문했을 때 고정된 fact checklist가 아니라 사용자 질문 자체로 관련 source unit을 찾고, 그 원문에 근거한 답변과 inline evidence를 보여주는 데까지를 닫는다.
...
- `/api/questions`는 사용자 질문을 retrieval query로 사용하고, 검색된 source unit 후보를 answer composer에 넘긴다.
- answer composer가 답변 draft와 근거 snippet을 제안하면 서버는 source unit 원문으로 snippet을 grounding한다.
- chat은 고정 fact target을 채우는 흐름이 아니다. 03의 fact candidate는 카드/정형 후보로 이어질 수 있는 내부 재료이지, 사용자-facing 채팅의 앞면을 지배하지 않는다.
...
- `apps/server/answers/library_chat.py`: 사용자 질문과 retrieved source unit 후보에서 grounded `ChatAnswer`를 만든다.
- `apps/server/api/routes/questions.py`: `/api/questions`가 ready material, question retrieval, `ChatAnswer`를 연결한다.
...
- LLM이 PDF 추출 공백 때문에 evidence snippet을 사람이 읽는 형태로 반환하면, 서버는 source unit 안에서 숫자 값이 순서대로 존재하는 좁은 원문 window로 evidence를 복구한다.
