...
먼저 고를 slice: PDF 선택/추가 -> PDF 텍스트 파싱 -> source unit/RAG search -> retrieval -> LLM/extractor 판단 -> 원문 근거 검증 -> 내부 FactCandidate를 거쳐 ChatAnswer로 투영 -> 인라인 근거.
...
| 3 | [Retrieval과 Evidence-backed fact candidate](03-evidence-backed-facts.md) | RAG가 고른 source unit 후보를 LLM/extractor가 읽고, backend가 원문 근거를 검증해 예약 확정서 제시는 `근거 있음`, source가 없는 체크인 시작 시각은 `근거 부족` 내부 FactCandidate로 만든 뒤 ChatAnswer 입력으로 넘긴다 |
...
| Retrieval / Evidence-backed fact candidate | 검색 후보를 ChatAnswer가 소비할 내부 후보와 근거 상태로 바꿔야 한다 | 삭제한 `src/server/trip-facts/extractTripFacts.ts`는 late arrival 값을 고정으로 만들었다 | Python backend가 RAG 후보를 LLM/extractor 판단에 넘기고, source unit 원문으로 evidence를 검증해 04가 소비할 내부 FactCandidate와 `EvidenceRef`를 만든다. source가 없는 체크인 시작 시각은 만들지 않는다 |
...
이번 slice에서 닫는 구현면은 `PDF 선택/추가 -> PDF 텍스트 파싱 -> source unit/RAG search -> retrieval -> LLM/extractor 판단 -> 원문 evidence 검증 -> 내부 FactCandidate를 거쳐 ChatAnswer로 투영 -> 인라인 근거`다. PDF 본문 없이 만든 답변이나, source unit 원문으로 되돌아갈 수 없는 `근거 있음`은 이 흐름을 대신할 수 없다.
