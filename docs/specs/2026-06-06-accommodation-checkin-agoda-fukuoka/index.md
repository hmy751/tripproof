...
먼저 고를 slice: PDF 선택/추가 -> PDF 텍스트 파싱 -> source unit/RAG search -> retrieval/grounding -> evidence-backed fact -> 채팅 답변 -> 인라인 근거.
...
| 3 | [Retrieval과 Evidence-backed fact](03-evidence-backed-facts.md) | retrieval candidate를 source unit 원문으로 grounding한 뒤 예약 확정서 제시는 `근거 있음`, source가 없는 체크인 시작 시각은 `근거 부족` fact로 만든다 |
...
| Retrieval / Evidence-backed fact | 검색 후보를 사용자-facing 후보와 상태로 바꿔야 한다 | 삭제한 `src/server/trip-facts/extractTripFacts.ts`는 late arrival 값을 고정으로 만들었다 | Python backend가 retrieval candidate를 source unit 원문으로 grounding해 `TripFact`와 `EvidenceRef`를 만든다. source가 없는 체크인 시작 시각은 만들지 않는다 |
...
| --- | --- | --- | --- |
| Agoda PDF에서 체크인 제시물 답변까지 | PDF 넣기 / 파싱 / source unit / RAG search / retrieval / evidence-backed fact / 채팅 / 인라인 근거 | 사용자가 PDF를 넣고 질문하면 예약 확정서 제시 안내가 source unit 원문에 근거해 `근거 있음`, PDF snippet으로 보인다 | 계정 연결, 장기 저장, 더 많은 PDF |
| 체크인 시작 시각 source 부재 처리 | PDF 넣기 / 파싱 / source unit / RAG search / retrieval / evidence-backed fact / 채팅 | 체크인 시작 시각을 뒷받침할 source unit이 없으면 값을 만들지 않고 `근거 부족`으로 멈춘다 | Agoda HTML 메일 본문 companion source |
...
이번 slice에서 닫는 구현면은 `PDF 선택/추가 -> PDF 텍스트 파싱 -> source unit/RAG search -> retrieval/grounding -> evidence-backed fact -> 상태 정규화 -> 채팅 답변 -> 인라인 근거`다. PDF 본문 없이 만든 답변이나, source unit 원문으로 되돌아갈 수 없는 `근거 있음`은 이 흐름을 대신할 수 없다.
