...
먼저 고를 slice: PDF 선택/추가 -> PDF 텍스트 파싱 -> source unit/RAG index -> retrieval/grounding -> evidence-backed fact -> 채팅 답변 -> 인라인 근거.
...
| 2 | [Source unit과 RAG index boundary](02-source-units-retrieval.md) | 파싱된 PDF 본문을 page/section/source unit으로 나누고, 검색용 index/embedding이 원문 locator로 되돌아가게 한다 |
...
| Source unit / RAG index | PDF 본문 전체가 아니라 어떤 원문 단위가 검색과 근거의 원천인지 추적해야 한다 | 01은 material text를 저장했고, 아직 source unit과 index boundary는 없다 | page/section/source unit을 만들고, index/embedding record가 source unit locator로 되돌아가게 한다 |
...
| --- | --- | --- | --- |
| Agoda PDF에서 체크인 제시물 답변까지 | PDF 넣기 / 파싱 / source unit / RAG index / retrieval / evidence-backed fact / 채팅 / 인라인 근거 | 사용자가 PDF를 넣고 질문하면 예약 확정서 제시 안내가 source unit 원문에 근거해 `근거 있음`, PDF snippet으로 보인다 | 계정 연결, 장기 저장, 더 많은 PDF |
| 체크인 시작 시각 source 부재 처리 | PDF 넣기 / 파싱 / source unit / RAG index / retrieval / evidence-backed fact / 채팅 | 체크인 시작 시각을 뒷받침할 source unit이 없으면 값을 만들지 않고 `근거 부족`으로 멈춘다 | Agoda HTML 메일 본문 companion source |
...
이번 slice에서 닫는 구현면은 `PDF 선택/추가 -> PDF 텍스트 파싱 -> source unit/RAG index -> retrieval/grounding -> evidence-backed fact -> 상태 정규화 -> 채팅 답변 -> 인라인 근거`다. PDF 본문 없이 만든 답변이나, source unit 원문으로 되돌아갈 수 없는 `근거 있음`은 이 흐름을 대신할 수 없다.

## 이번 AC

1. 사용자가 Agoda 예약 확정 PDF 하나를 넣을 수 있다.
2. 제품은 PDF 본문을 읽고 자료함에 파싱 완료 상태로 보여준다.
3. 제품은 PDF 본문에서 source unit과 검색용 index record를 만들고, 검색 결과가 source unit locator로 되돌아가게 한다.
...
3. source unit과 index record가 PDF page/locator/snippet으로 되돌아갈 수 있는지 확인한다.
