...
상태: 구현된 하위 작업 spec. `POST /api/materials`의 upload, parse, source unit/embedding record 생성, retrieval repository upsert, ready/failed 경계를 내부 observation record로 남기는 기준을 정한다.

## 왜 지금

자료함 질문 결과를 나중에 설명하려면 답변 경로만 보면 부족하다. 질문에 들어간 ready material이 어떤 업로드, parse 결과, source unit, embedding record 상태에서 만들어졌는지 먼저 확인할 수 있어야 한다.
...
- upload가 어떤 파일명, content type, 크기로 들어왔는가.
- PDF parse가 성공했는가, 실패했다면 어떤 failure kind인가.
- parse 성공 시 source unit이 몇 개 만들어졌는가.
...
- record는 upload, parse, source unit build, embedding record build, retrieval repository upsert, final material status 경계를 담는다.
...
- successful upload record는 `materialId`, final status, page count, source unit count, embedding status counts를 남길 수 있다.
...
- source unit text와 embedding vector는 기본 record에 넣지 않는다. 필요하면 id/count/status summary만 남긴다.
...
- `apps/server/materials/observation.py`: material upload observation record, step model, sink protocol, no-op/in-memory sink를 정의한다.
...
| --- | --- | --- | --- |
| Material upload observation model | 한 업로드 요청의 관측 결과를 안정된 형태로 남겨야 한다 | `materials/observation.py`에 ordered step record가 있다 | upload summary, phase outcomes, counts, final status, failure kind를 담는 내부 record가 있다 |
| Observation sink | record 생성과 저장 방식을 product logic에서 분리해야 한다 | no-op sink와 테스트용 in-memory sink가 있다 | 기본 no-op 또는 in-memory sink가 있어 비활성화 시 응답이 바뀌지 않는다 |
...
1. 정상 PDF 업로드는 기존 `ready` material 응답을 유지하면서 upload, parse success, source unit count, embedding status counts, final status `ready`를 담은 internal observation record를 만든다.
...
| Step | 남기는 facts | 실패 예 |
| --- | --- | --- |
| `upload` | `file_name`, `content_type`, `size_bytes`, `size_limit_bytes` | `unsupported_file`, `size_limit_exceeded` |
...
| `retrieval_repository_upsert` | 없음 | `repository_upsert_failed` |

`source_unit_build`는 현재 구현에서 chunking 경계다. PDF parse 결과의 text를 `SourceUnit`으로 나누는 단계이며, source unit text 자체는 observation record에 넣지 않는다.

기본 sink는 no-op이고, 테스트에서는 in-memory sink로 record를 확인한다. sink가 실패해도 product 응답과 저장 흐름을 바꾸지 않는다.

## 구현 중 결정

- 내부 record를 LangSmith보다 먼저 둔다. LangSmith는 product 관측 기준을 정하는 주체가 아니라, 내부 record를 외부 trace로 내보내는 선택적 sink/exporter다.
- record는 flat field 묶음보다 ordered step 구조로 둔다. 이후 LangSmith/OpenTelemetry 계층으로 보낼 때 step을 trace/run/event로 매핑하기 쉽고, `POST /api/materials` 흐름도 코드처럼 읽힌다.
- `validation`은 별도 step으로 두지 않는다. PDF 여부와 size limit은 독립 처리 단계가 아니라 upload 수용 실패에 가까우므로 `upload` step의 failure kind로 남긴다.
...
- step facts는 allowlist 방식으로 제한한다. 원본 PDF, 추출 전문, source unit text, embedding vector, provider raw response, stack trace가 들어갈 경로를 만들지 않는다.
