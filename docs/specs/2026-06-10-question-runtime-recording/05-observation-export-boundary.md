...
상태: 작성된 하위 작업 spec. 내부 material/question observation record와 runtime config snapshot을 no-op, local artifact, LangSmith 같은 export sink로 내보내기 전의 계약을 정한다. 구현은 local artifact exporter부터 시작하고, LangSmith adapter는 후속 작업으로 둔다.
...
이 slice는 내부 record를 바꾸는 작업이 아니라, 내부 record를 소비하는 export boundary를 잡는 작업이다. 먼저 local artifact exporter로 파일에 떨어지는 JSON-safe payload를 확인하고, 그 다음 같은 payload를 LangSmith trace/span/metadata로 매핑한다.

## 사용자 장면

사용자가 PDF 자료를 업로드하고 자료함에 질문한다. API 응답은 기존처럼 material과 question response만 반환한다.

개발자는 API 응답에 debug field를 추가하지 않고도, 요청 뒤에 남은 local artifact나 이후 LangSmith trace에서 다음을 확인할 수 있어야 한다.
...
- export payload가 source text, answer body, raw provider payload, embedding vector, secret을 포함하지 않는가.
...
- no-op, local artifact, LangSmith의 책임을 분리한다.
- 첫 구현은 local artifact exporter로 시작해 payload shape와 redaction boundary를 파일로 검증할 수 있게 한다.
...
- local artifact exporter는 configured directory에 JSONL 같은 append-friendly 파일로 기록한다. 기본 product path는 no-op이어야 하며, repo-local 출력 경로를 쓰는 구현에서는 해당 경로를 git ignore 대상에 둔다.
...
- raw PDF, source unit text, retrieval candidate 전문, answer body 전문, provider raw response, embedding vector, secret을 저장하지 않는다.
...
- `apps/server/materials/observation.py`: material upload record, operation-specific sink, no-op/in-memory sink, emit helper를 정의한다.
- `apps/server/questions/observation.py`: question answer record, operation-specific sink, no-op/in-memory sink, emit helper를 정의한다.
...
- `apps/server/app.py`: 기본 no-op observation sink를 app state에 연결한다.
...
| Record projection | 내부 record object를 그대로 외부로 흘리면 raw/sensitive field 판단이 약해진다 | internal record는 safe facts allowlist를 갖지만 export 전용 redaction은 없다 | material/question record를 export-safe dict로 변환하고, file name 같은 field는 external summary로 줄인다 |
...
| Operation-specific bridge | 기존 product path를 크게 흔들지 않고 exporter를 붙여야 한다 | `record_material_upload`, `record_question_answer`가 route에서 호출된다 | material/question observation sink implementation이 record를 projection해 common exporter에 넘긴다 |
| No-op exporter | export가 꺼진 환경에서도 product path가 동일해야 한다 | 기본 observation sink는 no-op이다 | export disabled 기본값이 no-op이고 응답/예외 동작이 바뀌지 않는다 |
| Local artifact exporter | LangSmith 전에 payload shape와 redaction을 눈으로 확인해야 한다 | 아직 파일 exporter가 없다 | configured directory에 JSONL artifact를 남기고, raw/source/provider/secret이 없는지 테스트로 확인한다 |
...
runtime config snapshot은 이미 secret/raw 값을 갖지 않는 safe snapshot이므로 export payload에 포함할 수 있다. 다만 prompt body, source text, vector, provider raw response, secret 값을 새로 채워 넣지 않는다.
...
- local artifact는 product source of truth가 아니며, API 응답이나 material/question 상태에서 참조하지 않는다.
...
- LangSmith input/output field에 question text, answer text, source text, raw provider payload를 기본으로 넣지 않는다.
...
3. no-op, local artifact, LangSmith adapter의 책임과 실패 동작이 구분된다.
4. local artifact exporter가 첫 구현 대상이며, LangSmith adapter는 같은 payload를 소비하는 후속 optional sink로 남는다.
...
4. local artifact exporter를 temp directory로 설정하고 material/question 요청 후 JSONL payload가 생성되는지 확인한다.
5. local artifact payload에 `observation_export.v1`, operation, record id, final status, steps, runtime config snapshot이 있는지 확인한다.
6. local artifact payload에 question text, answer body, source unit text, raw provider payload, embedding vector, secret, 원문 file name이 없는지 확인한다.
7. local artifact 쓰기 실패를 주입해도 public response와 기존 exception propagation이 바뀌지 않는지 확인한다.

## 남은 판단

- local artifact 기본 directory 이름과 env config 이름.
