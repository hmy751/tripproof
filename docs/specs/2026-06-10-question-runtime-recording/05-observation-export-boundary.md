...
- `apps/server/observations/export.py`: 공통 export envelope, material/question projection, no-op/local artifact exporter, operation-specific bridge sink를 정의한다.
...
| Local artifact exporter | LangSmith 전에 payload shape와 redaction을 눈으로 확인해야 한다 | `LocalArtifactObservationExporter`가 configured directory에 JSONL로 append한다 | configured directory에 JSONL artifact를 남기고, raw/source/provider/secret이 없는지 테스트로 확인한다 |
...
2026-06-11 현재 이 slice는 `apps/server/observations/export.py`, app factory wiring, local artifact test로 구현됐다.
...
local artifact projection은 내부 record의 step tree와 runtime config snapshot을 유지하되, `upload_snapshot.file_name`은 원문 파일명 대신 `file_name_present`, `file_extension` summary로 줄인다. 테스트는 export JSONL에 question text, answer body, source text, 원문 file name이 없는지 확인한다.
