...
- 첫 구현은 local observation artifact exporter로 시작해 payload shape와 redaction boundary를 파일로 검증할 수 있게 한다.
...
- `apps/server/observations/export.py`: 공통 export envelope, material/question projection, no-op/local observation artifact exporter, operation-specific bridge sink를 정의한다.
...
| Local observation artifact exporter | LangSmith 전에 payload shape와 redaction을 눈으로 확인해야 한다 | `LocalArtifactObservationExporter`가 configured directory에 JSONL로 append한다 | configured directory에 observation/export JSONL artifact를 남기고, raw/source/provider/secret이 없는지 테스트로 확인한다 |
...
2026-06-11 현재 이 slice는 `apps/server/observations/export.py`, app factory wiring, local observation artifact test로 구현됐다.
