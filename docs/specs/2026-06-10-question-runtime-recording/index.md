...
상태: 진행 중인 작업 spec. 01 prompt document runtime, 02 material upload observation, 03 question execution observation, 04 runtime config snapshot은 구현된 기준으로 정렬됐다. 05 observation export boundary는 구현 전 export 계약으로 작성됐다. LangSmith는 내부 record/snapshot 이후의 선택적 export 계층으로 둔다.
...
| Observation export boundary | 내부 record를 no-op/local/LangSmith로 내보내는 운반 경로가 필요하다 | 05에서 export envelope/projection/sink 책임을 작성했다. 구현은 아직 없다 | 내부 record shape를 흔들지 않는 JSON-safe export payload와 no-op/local/LangSmith 책임을 닫는다 |
| LangSmith export | 로컬 record만으로는 요청 단위 tree와 latency를 보기 어렵다 | 현재 dependency와 trace wrapper가 없다. 05의 export payload를 소비하는 후속 adapter로 둔다 | 내부 observation/snapshot export payload를 LangSmith로 내보내고, 꺼진 환경에서는 no-op로 통과한다 |
...
02 [Material upload 관측 record](02-material-upload-observation.md)는 구현됐다. 현재 구현은 `material_upload`를 parent/leaf step으로 남기고, LangSmith adapter는 아직 붙이지 않는다.
...
05 [Observation export boundary](05-observation-export-boundary.md)는 구현 전 spec으로 작성됐다. 내부 record와 runtime config snapshot을 export payload로 projection하고, no-op/local artifact/LangSmith의 책임을 분리한다.

다음 구현 단위는 local artifact exporter다. LangSmith adapter는 내부 record와 runtime config snapshot이 export payload로 닫힌 뒤 optional sink로 연결한다.
...
7. LangSmith adapter가 붙는 이후 export slice에서는 같은 export payload가 material/question trace로 export되는지 확인 항목으로 둔다.
