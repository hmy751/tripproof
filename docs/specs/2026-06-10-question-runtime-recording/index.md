...
상태: 진행 중인 작업 spec. 01 prompt document runtime, 02 material upload observation, 03 question execution observation, 04 runtime config snapshot, 05 observation export boundary/local artifact exporter, 06 LangSmith observation adapter, 07 request/correlation id, 08 eval run correlation artifact는 구현된 기준으로 정렬됐다.
...
| Eval run record | 제품 동작을 반복 관찰해야 한다 | 08에서 `eval/question_runtime_recording_smoke.py`가 run artifact와 local observation JSONL을 같은 correlation id로 묶는다 | product entry point, config snapshot, observed answer, failure, next verification point를 남길 수 있게 한다 |
...
08 [Eval run correlation artifact](08-eval-run-correlation-artifact.md)는 구현됐다. `eval/question_runtime_recording_smoke.py`는 product material/question entry point를 호출하고, run artifact top-level `correlation_id`와 local observation JSONL의 question export `correlation_id`가 같은지 확인한다. 이 run은 metric score가 아니라 request/correlation/export 연결 smoke다.
...
8. eval smoke run artifact는 top-level `correlation_id`와 local observation JSONL record의 `correlation_id`를 연결하되 raw source/question/LLM payload를 저장하지 않는다.
...
- eval run artifact에 LangSmith trace URL/id를 나중에 붙일지.
