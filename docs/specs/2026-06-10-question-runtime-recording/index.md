...
상태: 진행 중인 작업 spec. 01 prompt document runtime, 02 material upload observation, 03 question execution observation, 04 runtime config snapshot, 05 observation export boundary/local artifact exporter, 06 LangSmith observation adapter는 구현된 기준으로 정렬됐다. 07 request/correlation id는 구현 전 계약으로 작성됐다.
...
- `apps/server/app.py`: app factory, CORS, observation exporter wiring의 경계다.
...
- `apps/server/observations/langsmith.py`: `observation_export.v1` payload를 LangSmith root run, child run, metadata, tag로 projection한다.
...
| Observation correlation id | upload trace와 question trace가 같은 사용자 흐름인지 묶어야 한다 | 07에서 request/correlation id 계약을 작성했고 구현은 아직 없다 | `ObservationExportEnvelope`와 response header에 request/correlation id를 연결하되 product JSON body는 유지한다 |
...
07 [Observation request/correlation id](07-observation-correlation-id.md)는 작성됐다. `request_id`는 HTTP 요청 단위, `correlation_id`는 사용자 흐름 단위로 구분하고, `material_id`는 자료 단위 id로 남긴다. product JSON body에는 id를 넣지 않고, response header와 export envelope, LangSmith metadata/tag로 연결하는 계약이다. 구현은 아직 하지 않았다.
...
- 프론트엔드가 언제 새 correlation id를 만들고 언제 기존 id를 이어 쓸지.
