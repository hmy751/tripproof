...
상태: 구현된 작업 spec. 01 prompt document runtime, 02 material upload observation, 03 question execution observation, 04 runtime config snapshot, 05 observation export boundary/local observation artifact exporter, 06 LangSmith observation adapter, 07 request/correlation id, 08 eval run correlation artifact는 구현된 기준으로 정렬됐다.

## 왜 지금

TripProof는 사용자가 자료를 넣고 자료함에 질문해 근거가 붙은 답변을 받는 제품 경로를 갖기 시작했다. 다음 문제는 자료 업로드와 질문 답변이 흔들렸을 때 어떤 파일 처리 결과, prompt, provider, retrieval 설정, retrieval 후보(RetrievedSource)로 그 결과가 만들어졌는지 나중에 확인하기 어렵다는 점이다.
...
- 어떤 retrieval 후보(RetrievedSource)가 answer composer의 `AnswerContext`에 들어갔는가.
...
- eval smoke artifact는 같은 `correlation_id`로 product request와 observation export를 다시 찾을 수 있게 한다.
...
- retrieval 후보(RetrievedSource)나 LLM raw payload를 기본 API 응답에 붙여 product answer처럼 만들지 않는다.
...
- raw LLM payload 저장과 장기 로그 보관은 이번 범위가 아니다. 이 spec의 payload 경계는 product runtime을 다시 설명하는 관측 신호의 운반 범위를 정하는 계약이다.
...
| 05 | [Observation export boundary](05-observation-export-boundary.md) | 내부 observation record와 runtime config snapshot을 no-op, local observation artifact, LangSmith 같은 export sink로 내보내는 계약을 정한다 |
...
| 07 | [Observation request/correlation id](07-observation-correlation-id.md) | request 단위와 사용자 흐름 단위를 구분해 local observation artifact, LangSmith, response header에서 같은 trace를 찾는 계약을 정한다 |
| 08 | [Eval run correlation artifact](08-eval-run-correlation-artifact.md) | eval smoke run artifact와 local observation JSONL을 같은 correlation id로 연결하는 기준을 정한다 |
...
- `apps/server/observations/export.py`: material/question observation record를 request/correlation id가 붙은 `observation_export.v1` payload로 projection하고 no-op/local observation artifact exporter를 제공한다.
...
- `apps/server/retrieval/search.py`: retrieval top-k, similarity threshold, vector 검색/lexical fallback 경계다.
...
- `eval/question_runtime_recording_smoke.py`: product material/question entry point를 호출하고 correlation id가 연결된 run artifact와 local observation JSONL을 만든다.
- `eval/find_observation_by_correlation.py`: correlation id로 local observation JSONL record를 다시 찾는 lookup helper다.
...
| Observation export boundary | 내부 record를 no-op/local/LangSmith로 내보내는 운반 경로가 필요하다 | 05에서 export envelope/projection/sink 책임을 작성하고 no-op/local observation artifact exporter를 구현했다 | 내부 record shape를 흔들지 않는 JSON-safe export payload와 no-op/local/LangSmith 책임을 닫는다 |
...
| Eval run record | 제품 동작을 반복 관찰해야 한다 | 08에서 `eval/question_runtime_recording_smoke.py`가 run artifact와 local observation JSONL을 같은 correlation id로 묶고, `eval/find_observation_by_correlation.py`가 같은 id로 record를 다시 찾는다 | product entry point, config snapshot, observed answer, failure, next verification point를 남길 수 있게 한다 |
...
05 [Observation export boundary](05-observation-export-boundary.md)는 구현됐다. 내부 record와 runtime config snapshot을 `observation_export.v1` payload로 projection하고, no-op/local observation artifact/LangSmith의 책임을 분리한다. local observation artifact exporter는 configured directory에 JSONL로 append하며, 원문 파일명은 extension/presence summary로 줄인다.

06 [LangSmith observation adapter](06-langsmith-observation-adapter.md)는 구현됐다. LangSmith adapter는 내부 record와 runtime config snapshot이 export payload로 닫힌 뒤 optional sink로 연결하며, 첫 mapping은 operation root run, synthetic parent/leaf child runs, step events를 기준으로 한다. Synthetic child runs는 observation tree를 LangSmith UI에 투영한 표현이며, 현재 step별 실제 latency나 duration을 의미하지 않는다. local observation artifact와 LangSmith가 모두 설정되면 같은 export envelope를 fanout으로 양쪽에 보낸다.
...
08 [Eval run correlation artifact](08-eval-run-correlation-artifact.md)는 구현됐다. `eval/question_runtime_recording_smoke.py`는 product material/question entry point를 호출하고, run artifact top-level `correlation_id`와 local observation JSONL의 question export `correlation_id`가 같은지 확인한다. `eval/find_observation_by_correlation.py`는 같은 id로 local observation record를 다시 찾는다. 이 run은 metric score가 아니라 request/correlation/export 연결 smoke다.
...
3. `POST /api/questions`는 ready material 선택, retrieval, `AnswerContext` assembly, prompt snapshot, composer call, answer projection, accepted/blocked 경계를 내부 observation record로 남길 수 있다.
4. material/question 실행에서 결과를 바꿀 수 있는 provider/model/retrieval/prompt 관련 runtime config snapshot을 만들 수 있어야 한다.
5. export boundary는 내부 observation/snapshot record를 JSON-safe payload로 projection하고, no-op/local observation artifact/LangSmith sink가 꺼져 있거나 실패해도 기존 material/question 응답 계약을 유지한다.
...
8. eval smoke run artifact는 top-level `correlation_id`와 local observation JSONL record의 `correlation_id`를 연결하고, lookup helper는 같은 id로 record를 다시 찾되 raw SourceUnit text, raw question, raw LLM payload를 저장하거나 출력하지 않는다.
...
6. local observation artifact exporter가 붙는 이후 export slice에서는 내부 record가 JSON-safe payload로 projection되고 raw SourceUnit text, provider payload, secret 값이 없는지 확인한다.
...
9. 응답 payload가 raw retrieval 후보(RetrievedSource), raw LLM JSON, eval score를 사용자-facing answer처럼 포함하지 않는지 확인한다.
...
11. 08 구현에서는 `uv run python eval/question_runtime_recording_smoke.py --correlation-id flow_eval_test --json`로 생성된 `run.json`과 local observation JSONL의 question record가 같은 `correlation_id`를 갖는지 확인한다.
12. `uv run python eval/find_observation_by_correlation.py flow_eval_test`로 local observation record를 다시 찾을 수 있는지 확인한다.
...
- eval run artifact나 lookup 결과에 LangSmith trace URL/id를 나중에 붙일지.
...
- local observation artifact 파일 rotation/cleanup과 artifact viewer를 언제 붙일지.
