...
상태: 구현된 작업 spec. 01 prompt document runtime, 02 material upload observation, 03 question execution observation, 04 runtime config snapshot, 05 observation export boundary/local artifact exporter, 06 LangSmith observation adapter, 07 request/correlation id, 08 eval run correlation artifact는 구현된 기준으로 정렬됐다.
...
8. eval smoke run artifact는 top-level `correlation_id`와 local observation JSONL record의 `correlation_id`를 연결하고, lookup helper는 같은 id로 record를 다시 찾되 raw source/question/LLM payload를 저장하거나 출력하지 않는다.
