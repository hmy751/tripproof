...
상태: 진행 중인 작업 spec. 01 prompt document runtime, 02 material upload observation, 03 question execution observation, 04 runtime config snapshot은 구현된 기준으로 정렬됐다. LangSmith는 내부 record/snapshot 이후의 선택적 export 계층으로 둔다.
...
다음 구현 단위는 export boundary다. LangSmith adapter는 내부 record와 runtime config snapshot이 닫힌 뒤 optional export sink로 연결한다.
