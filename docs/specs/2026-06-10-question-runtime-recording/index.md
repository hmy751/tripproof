# 자료와 질문 실행 조건의 LangSmith 관측 기록

작성일: 2026-06-10

상태: 작업 spec. 자료 업로드와 자료함 질문 응답 경로를 prompt/config snapshot과 LangSmith 관측 기록으로 다시 확인 가능하게 만든다.
...
- LangSmith trace는 product 경로 옆에서 생성되며, product code가 eval run artifact나 fixture에 의존하지 않는다.
- 기본 material/question 응답 계약은 유지하고, raw retrieval/debug 정보는 사용자-facing 응답으로 노출하지 않는다.

## Rules

- `/api/materials`의 제품 흐름은 `upload -> parse -> source unit/embedding records -> ready/failed material -> 화면`이다. snapshot과 LangSmith trace는 이 흐름을 관찰한다.
- `/api/questions`의 제품 흐름은 `ready material -> retrieval -> answer composer -> grounded ChatAnswer -> 화면`이다. snapshot과 LangSmith trace는 이 흐름을 관찰한다.
...
| --- | --- | --- | --- |
| Material ingest observation | 질문 결과가 어떤 material 처리 결과에서 나왔는지 추적해야 한다 | `/api/materials`는 parse와 `store.add_ready`를 호출하지만 관측 기록은 없다 | material upload, parse success/failure, source unit/embedding record 생성 경계를 LangSmith trace로 본다 |
...
| LangSmith export | 로컬 기록만으로는 요청 단위 tree와 latency를 보기 어렵다 | 현재 dependency와 trace wrapper가 없다 | LangSmith가 켜진 환경에서는 material/question trace가 생기고, 꺼진 환경에서는 no-op로 통과한다 |
...
다음 slice는 `LangSmith no-op 안전 wrapper -> material upload trace -> question 실행 config/prompt snapshot -> /api/questions trace`다.
...
1. LangSmith가 설정된 환경에서는 `POST /api/materials`와 `POST /api/questions`의 요청 단위 trace가 생긴다.
...
4. LangSmith export가 꺼져 있어도 기존 material/question 응답 계약은 유지되고, raw debug나 eval 기록을 포함하지 않는다.
...
4. LangSmith가 켜진 환경에서 material/question trace와 active prompt version, config snapshot이 기록 가능한지 확인한다.
5. LangSmith가 꺼진 환경에서도 같은 product path가 동작하는지 확인한다.
6. 응답 payload가 raw retrieval candidate, raw LLM JSON, eval score를 사용자-facing answer처럼 포함하지 않는지 확인한다.
