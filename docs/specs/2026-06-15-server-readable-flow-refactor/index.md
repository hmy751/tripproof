...
- Supabase retrieval backend의 vector similarity source of truth는 database RPC `match_tripproof_source_units`가 반환하는 `similarity`다. Python `cosine_similarity` helper는 in-memory repository와 local vector fallback용이며 Supabase RPC similarity를 대체하지 않는다.
