...
- 입력 정규화와 HTTP 오류 변환.
- product 흐름의 실행 순서.
- retrieval record 생성과 vector 검색/lexical fallback 흐름.
...
- 질문 응답 경로는 ready material selection, retrieval record load, vector 검색/lexical fallback, answer context assembly, composer call, answer projection 경계를 읽을 수 있다.
...
- 특정 파일 분리 자체를 product contract로 고정하지 않는다. 이번 갱신에서 완료된 분리는 아래 update에 따르고, 남은 retrieval/answer/observation 추가 분리는 후보로만 둔다.
...
| `d8b968e` | 답변 context 타입명을 정리해 retrieval 결과가 answer composer 입력 context로 넘어가는 층위를 드러낸다. 현재 코드 이름은 `AnswerContext`, `RetrievedSource`다 | `apps/server/retrieval/models.py`, `apps/server/retrieval/search.py`, `apps/server/answers/library_chat.py` |
...
| `e6ec50d` | retrieval 흐름을 trace와 함께 분리한다 (이후 strategy 라벨은 제거됨. 현재 `SourceRetrievalTrace`는 vector 시도/fallback 여부와 candidate count를 boolean으로 남긴다) | `apps/server/retrieval/search.py`, `apps/server/tests/test_retrieval_search.py` |
...
## 2026-06-15 server boundary refactor update

이번 갱신은 새 제품 기능이 아니라, 위 readable-flow 기준을 현재 서버 코드 경계에 맞게 더 좁히는 작업이다. 과거 작업 기록을 대체하지 않고, 후속 refactor가 유지해야 할 product-facing 계약과 backend 책임 경계를 추가 기준으로 남긴다.

### 확정된 계약

- `materialIds`가 생략되거나 `null`이면 전체 ready material을 대상으로 한다.
- `materialIds: []`는 명시적 빈 선택이다. 이 경우 question response는 `blocked`가 되고 retrieval과 answer composer를 시작하지 않는다.
- ready material은 source unit 생성, embedding record 생성, retrieval repository upsert가 모두 끝난 뒤에만 store에 publish된다. 이 중 하나라도 실패하면 ready material로 노출되지 않아야 한다.
- store, use case, answer composer는 Pydantic API response schema를 내부 result type으로 반환하지 않는다. API schema 변환은 route/schema adapter 경계에서 수행한다.
- Supabase retrieval backend의 vector similarity source of truth는 database RPC `match_tripproof_source_units`가 반환하는 `similarity`다. Python `cosine_similarity` helper는 in-memory repository(vector 검색의 test/local 구현)용이며 Supabase RPC similarity를 대체하지 않는다.
- local observation artifact와 LangSmith adapter는 같은 `ObservationExportEnvelope` 의미를 소비해야 한다. `request_id`, `correlation_id`, `correlation_id_source`가 sink별로 빠지거나 서로 다르게 해석되면 안 된다.

### 현재 구현 경계

- observation export의 request context, envelope, serializer, redaction, sink, step helper를 별도 module로 분리했다. `apps/server/observations/export.py`는 compatibility re-export 경계로 남긴다.
- material ready ingestion에서 source unit 생성, embedding record 생성, retrieval upsert를 `MaterialIngestionPipeline`으로 분리했다. `MaterialStore`는 material 저장/조회와 publish 순서에 집중한다.
- question answer use case에서 material scope 선택, retrieval record/context 조회, accepted/blocked response projection을 별도 helper 경계로 분리했다.
- answer composer의 domain result와 API response schema를 분리하고, LLM answer item payload normalization을 별도 module로 옮겼다.
- Python-side vector similarity 계산을 공통 helper로 모았지만, Supabase retrieval path의 DB-side similarity 계약은 유지했다.
...
5. `retrieve_context_with_trace`가 source retrieval trace(vector 시도/fallback 여부, candidate count)와 `AnswerContext`를 함께 반환한다.
6. answer composer는 `AnswerContext`를 받아 domain `ChatAnswer`를 만든다.
7. use case는 accepted `QuestionAnswerResult`를 만들고 answer projection fact를 observation에 남긴다. route/schema adapter가 이를 public `QuestionResponse`로 변환한다.
...
| Retrieval boundary | vector 시도 여부, fallback 여부, candidate count가 retrieval trace로 남고 answer composer에는 `AnswerContext`가 넘어간다 | retrieval ranking, prompt rendering, answer validation이 한 함수에서 섞인다 |
...
5. AI/RAG 경계에서 vector 검색/lexical fallback, answer context, payload normalization, evidence grounding, observation fact projection이 서로 섞이지 않아야 한다.
...
- `apps/server/materials/observation.py`, `apps/server/questions/observation.py`는 common step helper를 공유하지만, record model, recorder, reporter, safe fact projection은 아직 각 파일에 함께 있다. observation step model이 더 커지면 model/recorder/reporter/projection 분리를 검토한다.
- `apps/server/retrieval/search.py`는 `_vector_search`(repository vector)와 `_lexical_fallback`을 갖고 있다. retrieval 흐름이 더 복잡해지면 vector 검색과 lexical fallback을 더 분리할 수 있다.
- `apps/server/answers/library_chat.py`는 payload normalization을 분리했지만 composer, prompt call, evidence grounding, display fallback을 함께 갖고 있다. evidence projection이나 display heuristic이 더 커지면 별도 module로 옮긴다.
...
- compatibility alias였던 `RetrievalCandidate = RetrievedSource`, `ContextPack = AnswerContext`는 용어 정합성 정리에서 제거했다.
