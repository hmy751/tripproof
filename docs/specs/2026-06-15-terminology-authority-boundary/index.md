...
| Extraction / candidate lane | `FactCandidate`, `FactProposal`, `FactTarget` | 정형 추출이나 후보 생성 내부 재료를 설명한다. 현재 Library Chat의 사용자-facing 기준어는 아니다. |
| Observation / export lane | `ObservationRecord`, `MaterialUploadObservationRecord`, `QuestionObservationRecord`, `ObservationExportEnvelope`, `trace`, `run`, `eval run artifact` | product path 실행 조건과 관측 산출물을 설명한다. |
...
| `FactCandidateResponse`와 `ChatAnswerItemResponse.from_fact`를 제거한다 | 현재 API 응답 경계에서 쓰지 않는 helper가 FactCandidate를 사용자-facing response처럼 보이게 한다 | `apps/server/schemas/facts.py`, `apps/server/schemas/answers.py` |
...
| artifact를 말할 때 | 제품 원본은 `Artifact / 자료`, eval/export 산출물은 `run artifact`처럼 쓴다. |
