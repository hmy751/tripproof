...
상태: draft sub-spec. answer certification boundary 이후, `04`가 드러낸 부족한 candidate/evidence role에 필요한 source unit 후보를 더 안정적으로 받도록 retrieval coverage를 개선하기 위한 product slice다.

이 문서의 중심은 retrieval 점수 자체가 아니다. `04`의 certification이 필요한 source unit 후보를 하위 요청별로 받을 수 있는지, 후보가 없을 때 어디서 빠졌는지 읽을 수 있게 만드는 것이다.

## 사용자 장면

사용자가 하나의 질문을 던졌고, `04`는 final state를 정하려면 어떤 evidence role이 충분하거나 부족한지 드러냈다. 이제 검색 단계가 그 role에 필요한 source unit 후보를 더 안정적으로 공급해야 한다.
...
네 role이 모두 같은 큰 source unit 하나만 받는다면 retrieval은 여전히 certification을 충분히 돕지 못한 것이다.

## Product 흐름

```text
04 candidate/certification 결과
...
이번 slice는 `role별 retrieval query`, `source unit 후보`, `candidate coverage 판단`까지의 계약을 다룬다. 최종 supported/needs_review/missing 정책은 `04`의 certification 계약을 따른다.
...
각 candidate/evidence role은 certification으로 넘어가기 전에 최소한 아래를 관찰할 수 있어야 한다.
...
4. 후보가 있는 role과 없는 role을 구분하고, 없는 경우 원인을 기록한다.
