...
상태: draft sub-spec. question decomposition과 sufficiency vertical 이후, 각 evidence requirement가 필요한 source unit 후보를 더 안정적으로 받도록 retrieval coverage를 개선하기 위한 product slice다.

이 문서의 중심은 retrieval 점수 자체가 아니다. `04`의 sufficiency check가 필요한 source unit 후보를 하위 요청별로 받을 수 있는지, 후보가 없을 때 어디서 빠졌는지 읽을 수 있게 만드는 것이다.
...
네 requirement가 모두 같은 큰 source unit 하나만 받는다면 retrieval은 여전히 sufficiency check를 충분히 돕지 못한 것이다.
...
이번 slice는 `requirement별 retrieval query`, `source unit 후보`, `candidate coverage 판단`까지의 계약을 다룬다. 최종 supported/needs_review/missing 정책은 `04`의 sufficiency 계약을 따른다.
...
lexical recall은 답을 만드는 규칙이 아니다. vector search가 놓칠 수 있는 라벨-값, 짧은 정책 제목, 비용 이름을 현재 source unit 후보군 안에서 다시 찾는 보조 장치다. 업체명, question id, eval expected cue, run artifact, source unit id를 미리 매핑해 후보를 꽂지 않는다. lexical/kind hit는 답변 값이나 supported 상태를 직접 만들 수 없고, candidate는 `04`의 evidence sufficiency를 통과하기 전까지 근거가 아니다.
...
7. 통과 판단은 candidate 필드 증가가 아니라 answer/evidence path 변화와 sufficiency 결과 변화로 한다.
