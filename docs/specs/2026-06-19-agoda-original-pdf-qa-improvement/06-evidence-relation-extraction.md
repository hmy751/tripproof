...
- **caveat 관계** — "이 조건이 이 값을 지배한다"는 관계. 단순히 한 page에 같이 있다거나 같은 kind라는 게 아니라, 의미상 그 조건이 그 값의 확정성을 좌우하는지다.
...
- 값에 그 값을 지배하는 caveat role이 붙어 있으면, 그 값은 단독으로 `supported`가 될 수 없다 — `needs_review`(또는 조건부)로 간다.
...
3. 값에 그 값을 지배하는 caveat role이 붙으면 그 값은 단독으로 `supported`가 되지 못한다(`needs_review` 또는 조건부).
...
3. 의미 층이 값 role과 그 값을 지배하는 조건 role을 분리해 내고, caveat 관계를 만드는지 확인한다.
