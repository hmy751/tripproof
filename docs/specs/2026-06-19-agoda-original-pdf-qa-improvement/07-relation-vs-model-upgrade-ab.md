...
상태: 측정 완료. **결정: 답변 호출과 분리된 별도 relation/caveat 호출은 제거한다(방향 B).** (실코드는 기본 `CAVEAT_EXTRACTOR_MODE=disabled`로 끈 상태이고 호출 경로 코드는 `relation.py`·`library_chat.py`에 잔존 — 실제 코드 제거 적용 시점·범위는 아래 "남은 판단"이다.) 단 이 결정의 근거는 깨끗한 단일 변수 비교가 아니라 confounded A/B + 계측 재실행이고, 정작 이 층이 존재한 이유였던 안전망(값이 자료에 적혀 있으나 보장은 아닌 경우를 잡는 것)은 **두 안 어디서도 해결되지 않았다.** 그 문제는 `08`(body 합성)·`05`(retrieval coverage)로 이월한다.
...
단, cue 채점이 얼마나 빡센지도 같이 봐야 한다. 채점은 필요한 표현이 답에 **글자 그대로(부분문자열)** 들어갔는지를 본다. 한 질문이 cue 여러 개를 요구하고 표현이 조금만 달라도 누락으로 잡혀, **통과 조건 자체가 구조적으로 엄격하다.** 따라서 낮은 pass(8문항 중 1~2개)는 답변 부실과 채점 엄격함이 섞인 값이고, **pass 수를 제품 품질로 직접 환산하면 안 된다.** 더구나 이 cue 게이트 자체가 글자 단위 키워드 게이트라, product가 줄이려는 글자 단위 brittleness를 eval 채점기가 동일하게 쓰고 있다 — eval은 방향 신호지 절대 기준이 아니다. 다만 신분증·결제카드처럼 표현 차이로 설명되지 않는 진짜 누락도 있어, 완성도가 실제 벽이라는 결론은 유지된다.
...
참고로 값이 `needs_review`에 닿는 경로는 셋이다 — LLM이 스스로 needs_review를 제안(자기 판정), 값이 원문에 글자로 없어 떨어짐(`value_not_grounded`, 우연), 조건이 grounding돼 강등(`limited_by_caveat`, 의도된 의미 안전망). 같은 `needs_review` 라벨이라도 도달 경로가 다르므로, needs_review 개수만으로 "안전망이 작동했다"고 읽으면 안 된다.

### (2) 계측 재실행 — A의 분리 검출기가 실제로 일했는지 직접 확인

표면 비교만으로는 A의 caveat이 답변 모델 inline에서 왔는지 별도 검출기에서 왔는지 구분되지 않았다. 그래서 A 구성을 caveat 출처가 관측에 기록되는 조건(관측 층 한정, 제품 응답 body 불변)에서 repeat 3으로 다시 측정했다(`Certification.caveat_source`).
...
따라서 원래 A run(run 23)의 P1-01 성공도 같은 inline 조건이 만들었을 가능성이 크다 — 이번 재실행에서 강한 검출기가 grounding되는 조건을 낸 유일한 경우가 false alarm이었기 때문이다. 즉 **A 실험은 "강한 분리 검출기의 가치"를 사실상 한 번도 검증하지 못한 셈이다.** 분리 층 제거 결정을 이 점이 더 받친다.
...
- **깨끗한 단일 변수 비교(C arm)는 돌리지 않았다.** 답변 모델을 강한 것으로 고정하고 별도 호출만 켰다 껐다 하는 비교가 분리 층의 한계 가치를 정면으로 잴 유일한 방법이다. 다만 계측 재실행이 "별도 층은 inline 위에 거의 못 보탠다"를 이미 시사한다.
...
- **이월한 inline 안전망은 `08`에서 실측됐다.** `08` 깨끗한 A/B(run 27, 답변 `qwen3:14b`, caveat 분리 호출 disabled)에서 P1-01이 `caveat_source: inline` + certify로 `needs_review`에 닿았다(`08` 참조) — 단 강한 답변 모델 의존이라 근본 문제는 여전히 미해결. (이 깨끗한 답변모델 A/B run 26·27은 분리 검출기 A의 confounded run 25와 섞지 않는다.)
