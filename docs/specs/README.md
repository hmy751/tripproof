...
작은 spec이나 이미 선택된 좁은 작업은 6-8을 생략할 수 있다. 큰 feature에서만 구현면을 펼쳐 slice 후보를 고른다.
...
| slice | 펼친 구현면을 얇게 관통하는 빌드 경로. 보통 한 필드, 한 상태, 한 실패 유형으로 좁혀 `자료 -> AI/adapter -> 상태 -> 화면`까지 닿게 한다. 작은 TODO나 부품 완성표가 아니다 |
...
`이번 AC`는 기본 1-3개로 제한한다. spec은 작업을 만들지 않고, 이미 선택한 사용자 장면을 좁히는 데만 쓴다. 큰 feature에서는 AC로 바로 뛰기 전에 구현면을 펼치고, slice 후보를 본 뒤, 먼저 고를 slice를 하나 정한다. slice 후보 목록은 진행 메모지 통과 기준이나 다음 작업 queue가 아니다. 비어 있어도 코딩을 막지 않고, 별도 work-map 파일은 만들지 않는다.
...
- **Slice 후보**: slice 후보는 구현면을 얇게 관통하는 product path다. 예를 들어 "체크인 시작 시간 답변"은 자료 입력, AI 후보, 정규화, 채팅, 인라인 근거를 지나지만 real ingestion과 real LLM은 stub으로 둘 수 있다.
...
Non-goals는 선택한 사용자 장면을 통과시키는 데 필요한 경로를 끊는 장치가 아니다. 줄이는 대상은 보통 기술 충실도와 확장 범위다. 실제 PDF/OCR parser, full LLM provider, 모든 필드, eval dashboard는 이번 slice 밖으로 둘 수 있지만, 자료 seed, deterministic adapter, stub state, 화면 표시처럼 사용자 장면이 지나갈 최소 경로는 살아 있어야 한다.
