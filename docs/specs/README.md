...
6. **구현면 펼치기**: 큰 feature라면 자료 입력, AI 후보, 정규화, 채팅, 근거, 카드, 대시보드처럼 사용자 장면이 실제로 지나갈 큰 면을 먼저 펼친다.
...
| 구현면 | feature를 화면까지 통과시키는 데 관련되는 큰 구현 면. 예: 자료 입력/ingest, AI 후보, 정규화, 채팅, 인라인 근거, 카드 초안, 대시보드 출처 |
...
아직 없는 product flow, eval run, before/after, metric, public case study를 완료된 proof처럼 쓰지 않는다. repo 밖 자료, 공고/포트폴리오 맥락, 대화 원문, 도구 이름도 product 근거처럼 남기지 않는다.

반대로, **spec 내용을 실제 eval run에 근거해 고칠 때는 그 판단을 뒷받침한 run 출처를 본문에 함께 남긴다.** `eval/runs/.../{run-id}/` 경로를 적고(run artifact가 gitignore여도 재접근 포인터다), 질문별 수치나 발췌 같은 상세는 `docs/implementation-notes/`에 보존해 거기로 가리킨다. run 묶음 폴더의 로컬 README는 gitignore라, 커밋되는 출처는 spec 본문과 implementation-note뿐이다. 그리고 **각 spec은 자기 slice가 돌린 run을 자기 문서에 담는다** — 예: `04`는 certification run(과잉강등 관측·해소 검증), `06`은 relation pass run을 담고, 다른 slice 문서나 index에만 떠넘기지 않는다(미구현이라 전용 run이 없으면 "eval 없음"을 명시한다).

TripProof의 기본 방향은 product-first다. product/eval 경계(eval은 관찰만, product가 eval sample·run artifact·metric에 의존 안 함)의 단일 기준은 `docs/engineering/architecture.md`이며, 여기서 재서술하지 않는다.
...
- **구현면 펼치기**: 큰 feature에서는 먼저 큰 면을 펼친다. 자료 입력, AI 후보, 정규화, 채팅, 근거, 카드, 대시보드처럼 사용자 장면이 지나갈 면을 본다. 이 단계는 작업표가 아니라 빠뜨린 면과 과하게 잘린 면을 확인하는 지도다.
...
구현면 펼치기는 제외할 것을 찾는 절차가 아니다. 큰 feature에서 `ingest`, `chat`, `AI 후보`, `정규화`, `근거`, `카드`, `대시보드` 같은 면을 먼저 펼치고, 현재 코드와 비교해 이미 되는 것과 stub으로 둘 것을 본다. 그 다음에야 slice 후보를 고른다.
