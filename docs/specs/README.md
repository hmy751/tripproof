...
spec은 승인 gate나 작업 목록이 아니라, 큰 작업에서 무엇을 통과로 볼지와 무엇을 Non-goals로 둘지를 맞추는 얇은 기준이다. 미리 써서 막는 gate가 아니라 개발하며 자라는 기준이다. 큰 feature는 여러 작업으로 이어질 제품 동작 기준이 필요할 때 feature spec으로 열고, 작은 보완도 독립 실패 유형이나 제품 경계가 있으면 작은 spec으로 열 수 있다. 다만 작은 수정마다 spec을 만드는 것은 아니다. 처음 읽을 때는 아래 순서로 본다.
