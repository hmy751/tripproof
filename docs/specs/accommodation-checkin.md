...
상태: 첫 product slice의 P0 product behavior acceptance. 아직 구현 완료나 eval 결과를 뜻하지 않는다. 이 문서는 구현 순서나 현재 작업 queue가 아니라, 숙소 체크인 slice가 어떤 사용자 동작을 통과해야 하는지 남긴다.
...
| user (직접 확인) | 직접 확인 | 사람이 원문에서 확인한 값을 채워 올린 카드. 근거가 부족/충돌이었거나, `근거 있음`이었어도 사람이 값을 고치면 이 출처로 바뀐다. |
...
`supported`의 사용자 문구 `근거 있음`은 근거 축과 결정 축에 같은 단어로 나타난다 — 근거 그대로 올린 카드는 같은 라벨을 유지하기 때문이다(분리 대상은 `확인 필요`⟂`직접 확인`이다). supported 답변을 사람이 값·이름을 고쳐 올리면 `직접 확인`으로 바뀐다.
...
- 근거 부족·충돌·민감이어도 카드 올리기를 막지 않고, 사람이 채워 올린 카드는 `직접 확인`으로 표시한다.
...
- `근거 있음` 답변을 사람이 값·이름을 고쳐 올리면 카드 출처를 `직접 확인`으로 바꿔 AI 근거 카드와 섞지 않는다.
- product는 eval fixture, run artifact, metric output을 알지 않는다.

## P0 관찰 case

아래 case는 product behavior를 관찰하기 위한 후보다. 현재 작업에서 전부 작성해야 하는 순서표가 아니라, acceptance를 확인할 때 다시 꺼내는 기준이다.

| Case | 의도 | acceptance 연결 |
| --- | --- | --- |
| P0-A happy path | 체크인 시작 시간과 늦은 도착 조건이 모두 명확함 (`근거 있음`) | supported 답변과 인라인 근거 |
| P0-B missing late arrival | 체크인 시간은 있지만 늦은 도착 조건이 없음 (`근거 부족`으로 멈춤, 직접 확인으로 올리기는 허용) | missing 답변과 직접 확인 카드 |
| P0-C multi-doc supplement | 예약 확인서와 호스트 메시지가 서로 보완 (전체 자료함 채팅이 두 자료를 함께 근거로 댐) | 여러 자료 근거 연결 |
| P0-D conflict check-in time | 예약 확인서와 호스트 메시지가 서로 다른 체크인 시작 시간을 말함 (`자료 충돌`) | conflict 답변과 양쪽 근거 |
| P0-E sensitive guard | 출입 코드나 예약번호가 보이더라도 자동 저장하지 않음 (`needs_review` + `sensitive`, snippet은 `[masked]`) | 민감 정보 자동 저장 방지 |
...
아래 신호는 구현을 막는 선행 gate가 아니라, 결과 검토 때 확인할 위험 신호다.
...
아래 질문은 현재 작업을 막지 않는다. 실제 구현 중 필요해질 때만 다시 연다.
