...
- 검증: `npm run black:check`, `npm test`를 통과했고, production-like eval을 `eval/runs/question-dataset/agoda-original-pdf-source-unit-structure-after-20260619-layout-v1-production/`에 생성했다. runtime은 Supabase retrieval, Ollama embedding, Ollama answer composer였다.
...
- 재측정: 기존 20260619 before/after 결과는 layout 필요성의 근거로 그대로 둔다. 같은 원문 PDF·같은 8문항·production-like runtime으로 reconciled 기반에서 baseline을 다시 측정했다. artifact는 `eval/runs/question-dataset/agoda-original-pdf-baseline-postreconcile-20260623-production/`.
